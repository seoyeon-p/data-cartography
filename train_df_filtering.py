...
def write_filtered_data(args, train_dy_metrics):
  ...
  if args.task_name == 'FEVER':
    file_extension = "train.jsonl"
  else:
    file_extension = 'train.tsv'
  original_train_file = os.path.join(os.path.join(args.data_dir, args.task_name), file_extension)
  #original_train_file = os.path.join(args.data_dir,args.task_name)
  train_numeric, header = read_data(original_train_file, task_name=args.task_name, guid_as_int=True)

  for fraction in [0.50]:#[0.01, 0.05, 0.10, 0.1667, 0.25, 0.3319, 0.50, 0.75]:
    ...
    selected_ids_list = []
    if args.task_name == 'FEVER':
      outfile_path = "train.jsonl"
      rest_filepath = "rest_train.jsonl"
    else:
      outfile_path = "train.txt"
      rest_filepath = "rest_train.txt"
    rest_file = open(os.path.join(outdir,rest_filepath),'w')

    with open(os.path.join(outdir, outfile_path), "w") as outfile:
      if args.task_name == 'FEVER':
        pass
      else:
        outfile.write(header + "\n")
        rest_file.write(header + "\n")
      # if you want to extract samples of the last 33% easy-to-learn, the latter line should be used to select the right range of samples
      selected = sorted_scores.head(n=num_samples+1)
      #selected = sorted_scores.tail(n=num_samples+1)
      if args.both_ends:
      ...
      for idx in selection_iterator:
        ...
        selected_ids_list.append(int(selected_id))
        if args.task_name in ["SNLI", "MNLI",'QQP','FEVER','SST','CoLA']:
        ...
        try:
          ...
          #print(selected.iloc[idx])
          if args.task_name == 'FEVER':
            #record = eval(train_numeric[item])
            json.dump(record,outfile)
            outfile.write("\n")
          else:
            detail = selected.iloc[idx]
            # guid, index, threshold_closeness, confidence, variabilitiy, correctness, forgetfulness
            info = []
            for item in detail:
              info.append(str(item))
            info = "\t".join(info)
            outfile.write(str(record)+"\t" +info + "\n")
        except:
          pass
    for item in train_numeric:
      if int(item) in selected_ids_list:
        pass
      else:
        if args.task_name == 'FEVER':
          if str(type(train_numeric[item])) != "<class 'dict'>":
            record = eval(train_numeric[item])
          else:
            record = train_numeric[item]
          json.dump(record,rest_file)
          rest_file.write('\n')
        else:
          rest_file.write(str(train_numeric[item])+"\n")
